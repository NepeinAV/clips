...
(defrule determine-engine-state ""
    ...
    (if (yes-or-no-p "Does the engine start (yes/no)?") then 
        (if (yes-or-no-p "Does the engine run normally (yes/no)?") then
...
(defrule determine-rotation-state ""
    ...
    (not (repair ?))
    =>
    (if (yes-or-no-p "Does the engine rotate (yes/no)?") then 
...
(defrule determine-gas-level ""
    ...
    (not (repair ?))
    =>
    (if (yes-or-no-p "Does the tank have any gas in it (yes/no)?") then 
...
(defrule determine-battery-state ""
    ...
    (not (repair ?))
    =>
    (if (yes-or-no-p "Is the battery charged (yes/no)?") then 
...
(defrule determine-low-output ""
    ...
    (not (repair ?))
    =>
    (if (yes-or-no-p "Is the output of the engine low (yes/no)?") then
...
(defrule determine-point-surface-state ""
    (or (and (working-state engine does-not-start) (spark-state engine irregular-spark)) (symptom engine low-output))
    (not (repair ?))
    =>
    (bind ?response (ask-question "What is the surface state of the points (normal/burned/contaminated)?" normal burned contaminated))
    (if (eq ?response burned) then
...
(defrule determine-conductivity-test "" 
    ...
    (not (repair ?))
    =>
    (if (yes-or-no-p "Is the conductivity test for the ignition coil positive(yes/no)?") then
...
(defrule determine-misfiring ""
    ...
    (not (repair ?))
    =>
    (if (yes-or-no-p "Does the engine misfire (yes/no)?") then
...
(defrule determine-knocking ""
    ...
    (not (repair ?))
    =>
    (if (yes-or-no-p "Does the engine knock (yes/no)?") then
